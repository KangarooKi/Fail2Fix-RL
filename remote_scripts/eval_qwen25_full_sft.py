...
def run(model, tokenizer, rows, max_new_tokens):
    outs = []
    for row in rows:
        enc = tokenizer(prompt_text(tokenizer, row["prompt"]), return_tensors="pt", truncation=True, max_length=2048)
...
def summarize(items):
    total = sum(x["reward"] for x in items)
    return {
        "reward_sum": total,
        "count": len(items),
        "reward_rate": total / max(1, len(items)),
...
def main():
    ...
    ap.add_argument("--sft-model", default="/root/autodl-tmp/learning_from_failure_exp/checkpoints/qwen25_05b_full_sft_2k")
    ap.add_argument("--data", default="/root/autodl-tmp/learning_from_failure_exp/data/qwen25_05b_eval_100.jsonl")
    ap.add_argument("--limit", type=int, default=50)
    ap.add_argument("--max-new-tokens", type=int, default=512)
    ap.add_argument("--output", default="/root/autodl-tmp/learning_from_failure_exp/reports/qwen25_05b_full_sft_eval.json")
    args = ap.parse_args()
    ...
    sft_model = load_model(args.sft_model)
