...
class IndexedCorrectionDataset(Dataset):
    def __init__(self, path, tokenizer, max_length=4096, index_path=None):
        self.path = str(path)
    ...
    def __getitem__(self, index):
        for shift in range(8):
            ...
            if item is not None:
                return item
        raise RuntimeError("Could not encode a usable correction sample.")

    def encode(self, row):
        prompt = (row.get("prompt") or row.get("sft_prompt") or "").strip()
        response = (row.get("response") or row.get("teacher_response") or "").strip()
...
def main():
    ...
    dataset = IndexedCorrectionDataset(args.data, tokenizer, max_length=args.max_length)
    eval_dataset = IndexedCorrectionDataset(args.eval_data, tokenizer, max_length=args.max_length) if args.eval_data else None
    ...
    metrics.update(
        {
            ...
            "save_total_limit": args.save_total_limit,
            "trainable": "full",
            "objective": "teacher_correction_sft",
        }
    )
    Path(args.output_dir).mkdir(parents=True, exist_ok=True)
    Path(args.output_dir, "correction_sft_metrics.json").write_text(json.dumps(metrics, ensure_ascii=False, indent=2), encoding="utf-8")
